 ...
 * peaks. This is the physically relevant split for the reco/reio
 * tomographic channels; using tau_rec cuts the recombination bump too
 * early and suppresses the reco contribution at low ell.
 *
 * If no separate reionization bump is found, fall back to tau_rec.
 */
static int transfer_visibility_split_tau(
  ...
  *tau_split = pth->tau_rec;
  ...
  class_test(reco_peak < 0,
             pth->error_message,
             "Could not identify the recombination visibility peak in transfer_visibility_split_tau.");
...
/**
 * Identify the actual recombination and reionization visibility peaks.
 *
 * These peak locations are used to label the reco/reio visibility split
 * and to set the corresponding low-ell distance scales, while the
 * active source split itself is performed with the full time history.
 */
static int transfer_visibility_peaks(
                                     struct thermodynamics * pth,
                                     int * index_peak_reco,
                                     int * index_peak_reio,
                                     double * z_peak_reco,
  ...
  *index_peak_reco = -1;
  *index_peak_reio = -1;
  *z_peak_reco = pth->z_rec;
  *z_peak_reio = pth->z_reio;
  *tau_peak_reco = pth->tau_rec;
  *tau_peak_reio = pth->conf_time_reio;
  *g_peak_reco = -1.0;
  *g_peak_reio = -1.0;
  *has_peak_reio = _FALSE_;
  ...
  class_test(reco_peak < 0,
             pth->error_message,
             "Could not identify the recombination visibility peak in transfer_visibility_reio_peak_tau.");
  ...
  *z_peak_reco = pth->z_table[reco_peak];
  *tau_peak_reco = pth->tau_table[reco_peak];
  *g_peak_reco = g_reco;

  if (reio_peak >= 0) {
    *index_peak_reio = reio_peak;
    *z_peak_reio = pth->z_table[reio_peak];
    *tau_peak_reio = pth->tau_table[reio_peak];
    *g_peak_reio = g_reio;
    *has_peak_reio = _TRUE_;
  }
 ...
 * locations; no Gaussian widths are used anywhere downstream.
 */
static int transfer_visibility_peak_windows(
                                            struct thermodynamics * pth,
                                            double * tau_peak_reco,
                                            double * sigma_reco,
                                            double * tau_peak_reio,
  ...
  *sigma_reio = 0.0;
  ...
  if (index_peak_reco >= 0) {
    g_half = 0.5 * g_peak_reco;
    index_left = index_peak_reco;
    while ((index_left > 0) &&
           (pth->thermodynamics_table[index_left*pth->th_size + pth->index_th_g] > g_half)) {
      index_left--;
    }
    index_right = index_peak_reco;
    while ((index_right < pth->tt_size - 1) &&
           (pth->thermodynamics_table[index_right*pth->th_size + pth->index_th_g] > g_half)) {
      index_right++;
    }

    tau_left = pth->tau_table[index_left];
    tau_right = pth->tau_table[index_right];
    fwhm = fabs(tau_right - tau_left);

    if ((fwhm <= 0.0) && (index_peak_reco > 0)) {
      fwhm = fabs(pth->tau_table[index_peak_reco] - pth->tau_table[index_peak_reco-1]);
    }
    if ((fwhm <= 0.0) && (index_peak_reco < pth->tt_size - 1)) {
      fwhm = fabs(pth->tau_table[index_peak_reco+1] - pth->tau_table[index_peak_reco]);
    }

    *sigma_reco = fwhm / (2.0 * sqrt(2.0 * log(2.0)));
  }

  if ((index_peak_reio >= 0) && (*has_peak_reio == _TRUE_)) {
    index_start = MIN(index_peak_reco, index_peak_reio);
    index_end = MAX(index_peak_reco, index_peak_reio);
    valley_index = index_start;

    for (index_th = index_start + 1; index_th <= index_end; index_th++) {
      if (pth->thermodynamics_table[index_th*pth->th_size + pth->index_th_g] <
          pth->thermodynamics_table[valley_index*pth->th_size + pth->index_th_g]) {
        valley_index = index_th;
      }
    }

    g_half = 0.5 * g_peak_reio;
    index_left = index_peak_reio;
    while ((index_left > 0) &&
           (index_left != valley_index) &&
           (pth->thermodynamics_table[index_left*pth->th_size + pth->index_th_g] > g_half)) {
      index_left--;
    }
    index_right = index_peak_reio;
    while ((index_right < pth->tt_size - 1) &&
           (index_right != valley_index) &&
           (pth->thermodynamics_table[index_right*pth->th_size + pth->index_th_g] > g_half)) {
      index_right++;
    }
    ...
    if ((fwhm <= 0.0) && (index_peak_reio > 0)) {
      fwhm = fabs(pth->tau_table[index_peak_reio] - pth->tau_table[index_peak_reio-1]);
    }
    if ((fwhm <= 0.0) && (index_peak_reio < pth->tt_size - 1)) {
      fwhm = fabs(pth->tau_table[index_peak_reio+1] - pth->tau_table[index_peak_reio]);
    }

    *sigma_reio = fwhm / (2.0 * sqrt(2.0 * log(2.0)));
  }
 ...
 * conformal time.
 */
static int transfer_find_nearest_source_tau_index(
                                                  struct perturbations * ppt,
                                                  double tau_peak,
                                                  int * index_tau_peak
                                                  ) {
  ...
  class_test((tau_peak < ppt->tau_sampling[0]) ||
             (tau_peak > ppt->tau_sampling[ppt->tau_size-1]),
             ppt->error_message,
             "Cannot localize transfer source at tau=%e outside sampled range [%e,%e].",
             tau_peak,
             ppt->tau_sampling[0],
             ppt->tau_sampling[ppt->tau_size-1]);

  *index_tau_peak = 0;
  best_distance = fabs(ppt->tau_sampling[0] - tau_peak);

  for (index_tau = 1; index_tau < ppt->tau_size; index_tau++) {
    distance = fabs(ppt->tau_sampling[index_tau] - tau_peak);
    if (distance < best_distance) {
      best_distance = distance;
      *index_tau_peak = index_tau;
    }
  }
...
int transfer_init(
  ...
  */
  ...
  if (ptr->transfer_verbose > 0) {
    if (ptr->has_tomo_peak_reio == _TRUE_) {
      ...
      fprintf(stdout,
              "[tomo visibility split] reco_peak: th_index=%d source_index=%d z=%g tau=%g g=%e | reio_peak: none found | split_tau=%g\n",
              index_th_peak_reco,
              index_tau_peak_reco,
              z_tomo_peak_reco,
              ptr->tau_tomo_peak_reco,
 ...
 class_run_parallel(with_arguments(pba,pth,ppt,ptr,ppr,index_q,tau_rec,tau_tomo_split,tp_of_tt,sources,sources_spline,tau_size_max,window,tau0,&BIS),
...
int transfer_indices(
  ...
  if (ppt->has_scalars == _TRUE_) {
    ...
    class_define_index(ptr->index_tt_t0_reco,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_t1_reco,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_t2_reco,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_t0_reio,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_t1_reio,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_t2_reio,   ppt->has_cl_cmb_temperature,      index_tt,1);
    class_define_index(ptr->index_tt_e_reco,    ppt->has_cl_cmb_polarization,     index_tt,1);
    class_define_index(ptr->index_tt_e_reio,    ppt->has_cl_cmb_polarization,     index_tt,1);
  ...
  */
  ...
  if (ppt->has_cls == _TRUE_) {
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      /* Removed invalid alpha block here */
    }
  ...
  for (index_md = 0; index_md < ptr->md_size; index_md++) {
    ...
    for (index_tt=0; index_tt<ptr->tt_size[index_md]; index_tt++) {
      ...
      if (index_md == ppt->index_md_scalars) {
        ...
        else if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t1))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t1;

        else if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t2))
        ...
        else if ((ppt->has_source_alpha == _TRUE_) && (index_tt == ptr->index_tt_alpha))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_alpha;

        else if ((ppt->has_source_alpha == _TRUE_) && (index_tt == ptr->index_tt_alpha_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_alpha;
        ...
        else if (_index_tt_in_range_(ptr->index_tt_rsd,     ppt->selection_num, ppt->has_nc_rsd))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_theta_m;

        else if (_index_tt_in_range_(ptr->index_tt_d0,      ppt->selection_num, ppt->has_nc_rsd))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_theta_m;

        else if (_index_tt_in_range_(ptr->index_tt_d1,      ppt->selection_num, ppt->has_nc_rsd))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_theta_m;

        else if (_index_tt_in_range_(ptr->index_tt_nc_lens, ppt->selection_num, ppt->has_nc_lens))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi_plus_psi;

        else if (_index_tt_in_range_(ptr->index_tt_nc_g1,   ppt->selection_num, ppt->has_nc_gr))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_psi;

        else if (_index_tt_in_range_(ptr->index_tt_nc_g2,   ppt->selection_num, ppt->has_nc_gr))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi;

        else if (_index_tt_in_range_(ptr->index_tt_nc_g3,   ppt->selection_num, ppt->has_nc_gr))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi_prime;

        else if (_index_tt_in_range_(ptr->index_tt_nc_g4,   ppt->selection_num, ppt->has_nc_gr))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi_plus_psi;

        else if (_index_tt_in_range_(ptr->index_tt_nc_g5,   ppt->selection_num, ppt->has_nc_gr))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi_plus_psi;

        else if ((ppt->has_cl_lensing_potential == _TRUE_) &&
                 (index_tt >= ptr->index_tt_lensing) &&
                 (index_tt < ptr->index_tt_lensing+ppt->selection_num))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_phi_plus_psi;
      }

      else if ((ppt->has_vectors == _TRUE_) && (index_md == ppt->index_md_vectors)) {
        ...
        else if ((ppt->has_cl_cmb_polarization == _TRUE_) && (index_tt == ptr->index_tt_b))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_p;
      }

      else if ((ppt->has_tensors == _TRUE_) && (index_md == ppt->index_md_tensors)) {
      ...
      class_test(tp_of_tt[index_md][index_tt] < 0,
                 ptr->error_message,
                 "No perturbation source correspondence found for mode index %d and transfer type index %d.",
                 index_md,
                 index_tt);
    }
  }
 ...
 * @param tau_tomo_split        Input: visibility-valley split time for reco/reio tomography
        ...
        for (index_tt = 0; index_tt < ptr->tt_size[index_md]; index_tt++) {
          ...
          if ((use_full_limber == _FALSE_) || (index_tt == ptr->index_tt_lcmb)) {
            ...
            for (index_l = 0; index_l < ptr->l_size[index_md]; index_l++) {
              ...
              {
                double ra_for_neglect;

                ra_for_neglect = (pba->conformal_age-tau_rec)*ptr->angular_rescaling;
                ...
                if ((index_tt == ptr->index_tt_t0_reco) ||
                    ...
                    (index_tt == ptr->index_tt_e_reco) ||
                    (index_tt == ptr->index_tt_alpha_reco)) {
                  ra_for_neglect = (pba->conformal_age-ptr->tau_tomo_peak_reco)*ptr->angular_rescaling;
                }

                if ((index_tt == ptr->index_tt_t0_reio) ||
                    (index_tt == ptr->index_tt_t1_reio) ||
                    (index_tt == ptr->index_tt_t2_reio) ||
                    (index_tt == ptr->index_tt_e_reio) ||
                    (index_tt == ptr->index_tt_alpha_reio)) {
                  ra_for_neglect = (pba->conformal_age-ptr->tau_tomo_peak_reio)*ptr->angular_rescaling;
                }

                /* For tomographic reco/reio channels, use the corresponding
                   emission distance in the low-l neglect criterion. Using the
                   recombination distance for reionization suppresses the large-scale
                   reionization bump too aggressively. */
...
int transfer_sources(
  ...
  if (index_md == ppt->index_md_scalars) {
    ...
    is_nonintegrated_nc =
      (_index_tt_in_range_(ptr->index_tt_density, ppt->selection_num, ppt->has_nc_density) ||
       _index_tt_in_range_(ptr->index_tt_rsd,     ppt->selection_num, ppt->has_nc_rsd) ||
       _index_tt_in_range_(ptr->index_tt_d0,      ppt->selection_num, ppt->has_nc_rsd) ||
       _index_tt_in_range_(ptr->index_tt_d1,      ppt->selection_num, ppt->has_nc_rsd) ||
       _index_tt_in_range_(ptr->index_tt_nc_g1,   ppt->selection_num, ppt->has_nc_gr) ||
  ...
  if (redefine_source == _TRUE_) {
    ...
    if (index_md == ppt->index_md_scalars) {
      ...
      else if (is_nonintegrated_nc == _TRUE_) {
      ...
      else if (is_integrated_nc == _TRUE_) {
  ...
  /** - case where we do not need to redefine */
    ...
    /* plain copy from input array to output array for full channels */
    for (index_tau=0; index_tau < ppt->tau_size; index_tau++) {
  ...
  /** - return tau_size value that will be stored in the workspace */
...
int transfer_can_be_neglected(
  ...
  *neglect = _FALSE_;
  ...
  if (_scalars_) {
    ...
    else if ((ppt->has_cl_cmb_polarization == _TRUE_) &&
             ((index_tt == ptr->index_tt_e) ||
              (index_tt == ptr->index_tt_e_reco) ||
              (index_tt == ptr->index_tt_e_reio)) &&
             (l < (k-ppr->transfer_neglect_delta_k_S_e)*ra_rec)) *neglect = _TRUE_;
    ...
    if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t1) && (l < (k-ppr->transfer_neglect_delta_k_V_t1)*ra_rec)) *neglect = _TRUE_;
             ...
             ((index_tt == ptr->index_tt_e) ||
              (index_tt == ptr->index_tt_e_reco) ||
              (index_tt == ptr->index_tt_e_reio)) &&
             (l < (k-ppr->transfer_neglect_delta_k_V_e)*ra_rec)) *neglect = _TRUE_;
    ...
    if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t2) && (l < (k-ppr->transfer_neglect_delta_k_T_t2)*ra_rec)) *neglect = _TRUE_;
             ...
             ((index_tt == ptr->index_tt_e) ||
              (index_tt == ptr->index_tt_e_reco) ||
              (index_tt == ptr->index_tt_e_reio)) &&
             (l < (k-ppr->transfer_neglect_delta_k_T_e)*ra_rec)) *neglect = _TRUE_;
...
int transfer_select_radial_function(
  ...
  *radial_type = SCALAR_TEMPERATURE_0;
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_alpha == _TRUE_) {
      if (index_tt == ptr->index_tt_alpha)
        *radial_type = SCALAR_TEMPERATURE_0;

      if (index_tt == ptr->index_tt_alpha_reco)
        *radial_type = SCALAR_TEMPERATURE_0;
