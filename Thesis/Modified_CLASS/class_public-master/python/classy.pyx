...
from math import ceil, cos, exp, floor, lgamma, log, pi, sin, sqrt
import numpy as np
...
def _rotation_wigner3j(j1, j2, j3, m1, m2, m3):
    """
    Exact Wigner 3j symbol computed with the Racah formula.

    This mirrors the helper that used to live in `source/harmonic.c`.
    """
    if m1 + m2 + m3 != 0:
        return 0.0
    ...
    phase = -1.0 if ((j1 - j2 + m3) % 2) else 1.0
    ...
    log_delta_den = lgamma(j1 + j2 + j3 + 2)
    log_delta = 0.5 * (log_delta_num - log_delta_den)

    log_sqrt_fac = 0.5 * (
        lgamma(j1 + m1 + 1) + lgamma(j1 - m1 + 1)
    ...
    t_min = max(0.0, float(j2 - j3 - m1), float(j1 - j3 + m2))
    t_max = min(float(j1 + j2 - j3), float(j1 - m1), float(j2 + m2))

    if t_min > t_max + 0.5:
        return 0.0

    value = 0.0
    for t_int in range(int(ceil(t_min)), int(floor(t_max + 0.5)) + 1):
        log_denom = (
            lgamma(t_int + 1)
            + lgamma(j1 + j2 - j3 - t_int + 1)
            + lgamma(j1 - m1 - t_int + 1)
            + lgamma(j2 + m2 - t_int + 1)
            + lgamma(j3 - j2 + m1 + t_int + 1)
            + lgamma(j3 - j1 - m2 + t_int + 1)
        )
        term = (-1.0 if (t_int % 2) else 1.0) * exp(log_prefactor - log_denom)
        value += term
...
@lru_cache(maxsize=None)
def _rotation_I_coupling(l, L1, L2, m1, m2, m3):
    """
    Coupling object I^{s1,s2,s3}_{l L1 L2} used in the birefringence sums.
    """
    w3j = _rotation_wigner3j(l, L1, L2, m1, m2, m3)
    if w3j == 0.0:
        return 0.0
    return sqrt((2.0 * l + 1.0) * (2.0 * L1 + 1.0) * (2.0 * L2 + 1.0) / (4.0 * pi)) * w3j


def _rotation_get_array(cl_inputs, key, size):
    if key not in cl_inputs:
        return np.zeros(size, dtype=np.double)

    arr = np.asarray(cl_inputs[key], dtype=np.double)
    if arr.ndim != 1 or arr.shape[0] != size:
        raise ValueError(
            "Expected '{0}' to have shape ({1},), got {2}".format(
                key, size, arr.shape))
    return arr


def _rotation_epoch(ell, ee, te, aa, ae, alpha0, bb=None):
    """
    Rotate one tomography block (reco or reio) with the same approximation
    previously used in the C harmonic module.
    """
    ell = np.asarray(ell, dtype=np.int64)
    size = ell.shape[0]

    if bb is None:
        bb = np.zeros(size, dtype=np.double)
    else:
        bb = np.asarray(bb, dtype=np.double)

    V_alpha = np.sum((2.0 * ell + 1.0) * aa / (4.0 * pi))
    ...
    ee_rot = np.zeros(size, dtype=np.double)
    bb_rot = np.zeros(size, dtype=np.double)
    eb_rot = np.zeros(size, dtype=np.double)
    te_rot = np.zeros(size, dtype=np.double)
    tb_rot = np.zeros(size, dtype=np.double)
    ...
    for l in range(2, lmax + 1):
        ...
        for L1 in range(2, lmax + 1):
            ...
            L2_max = min(lmax, l + L1)

            for L2 in range(L2_min, L2_max + 1):
                Caa_L2 = aa[L2]
                CaE_L2 = ae[L2]

                if Caa_L2 == 0.0 and (CaE_L1 == 0.0 or CaE_L2 == 0.0):
                    continue

                I220 = _rotation_I_coupling(l, L1, L2, -2, 2, 0)
                if I220 == 0.0:
                    continue

                cross_term = 0.0
                if CaE_L1 != 0.0 and CaE_L2 != 0.0:
                    I20m2 = _rotation_I_coupling(l, L1, L2, -2, 0, 2)
                    cross_term = CaE_L1 * CaE_L2 * I20m2

                parity = 1.0 if ((l + L1 + L2) % 2 == 0) else -1.0

                sum_EE += I220 * (
                    Caa_L2 * I220 * (
                        CEE_L1 * (1.0 - parity * cos4)
                        + CBB_L1 * (1.0 + parity * cos4)
                    ) - cross_term * (1.0 - parity * cos4)
                )
                ...
                sum_EB += I220 * (
                    Caa_L2 * I220 * (
                        CBB_L1 * (-parity * sin4)
                        - CEE_L1 * (parity * sin4)
                    ) - cross_term * (-parity * sin4)
                )

        iso_EE = (1.0 - 4.0 * V_alpha) * (EE_l * cos2 * cos2 + BB_l * sin2 * sin2)
        iso_BB = (1.0 - 4.0 * V_alpha) * (BB_l * cos2 * cos2 + EE_l * sin2 * sin2)
        iso_EB = 0.5 * (1.0 - 4.0 * V_alpha) * (EE_l - BB_l) * sin4

        ee_rot[l] = iso_EE + (2.0 / (2.0 * l + 1.0)) * sum_EE
        bb_rot[l] = iso_BB + (2.0 / (2.0 * l + 1.0)) * sum_BB
        eb_rot[l] = iso_EB + (2.0 / (2.0 * l + 1.0)) * sum_EB
        te_rot[l] = (1.0 - 2.0 * V_alpha) * cos2 * TE_l
        tb_rot[l] = (1.0 - 2.0 * V_alpha) * sin2 * TE_l
...
def rotate_birefringence_cl(cl_inputs, lmax=None):
    """
    Rotate the unlensed birefringence inputs on the Python side.

    The implementation mirrors the second-order Wigner-3j rotation that used to
    live in `source/harmonic.c`:
    - reco/reio blocks are rotated independently
    - cross-epoch terms are neglected
    - unlensed B modes default to zero unless `bb_reco` / `bb_reio` are provided

    The `at`, `at_reco`, and `at_reio` spectra returned by `rotation_cl_inputs()`
    are not used here, matching the current CLASS-side approximation.
    """
    if 'ell' not in cl_inputs:
        raise ValueError("Missing required key 'ell' in cl_inputs.")
    if 'alpha0_reco' not in cl_inputs or 'alpha0_reio' not in cl_inputs:
        raise ValueError("Missing 'alpha0_reco' or 'alpha0_reio' in cl_inputs.")

    ell_full = np.asarray(cl_inputs['ell'], dtype=np.int64)
    if ell_full.ndim != 1:
        raise ValueError("Expected 'ell' to be one-dimensional.")

    lmax_available = ell_full.shape[0] - 1
    if lmax is None or lmax < 0:
        lmax_use = lmax_available
    else:
        lmax_use = min(int(lmax), lmax_available)

    ell = ell_full[:lmax_use + 1]
    expected_ell = np.arange(lmax_use + 1, dtype=np.int64)
    if not np.array_equal(ell, expected_ell):
        raise ValueError("Expected 'ell' to run contiguously from 0 to lmax.")

    size = lmax_use + 1

    ee_reco = _rotation_get_array(cl_inputs, 'ee_reco', size)
    ee_reio = _rotation_get_array(cl_inputs, 'ee_reio', size)
    te_reco = _rotation_get_array(cl_inputs, 'te_reco', size)
    te_reio = _rotation_get_array(cl_inputs, 'te_reio', size)
    aa_reco = _rotation_get_array(cl_inputs, 'aa_reco', size)
    aa_reio = _rotation_get_array(cl_inputs, 'aa_reio', size)
    ae_reco = _rotation_get_array(cl_inputs, 'ae_reco', size)
    ae_reio = _rotation_get_array(cl_inputs, 'ae_reio', size)
    bb_reco = _rotation_get_array(cl_inputs, 'bb_reco', size)
    bb_reio = _rotation_get_array(cl_inputs, 'bb_reio', size)

    reco = _rotation_epoch(
        ell,
        ee_reco,
        te_reco,
        aa_reco,
        ae_reco,
        float(cl_inputs['alpha0_reco']),
        bb_reco)
    reio = _rotation_epoch(
        ell,
        ee_reio,
        te_reio,
        aa_reio,
        ae_reio,
        float(cl_inputs['alpha0_reio']),
        bb_reio)

    return {
        'ell': ell.copy(),
        'ee_rot': reco['ee_rot'] + reio['ee_rot'],
        'bb_rot': reco['bb_rot'] + reio['bb_rot'],
        'eb_rot': reco['eb_rot'] + reio['eb_rot'],
        'te_rot': reco['te_rot'] + reio['te_rot'],
        'tb_rot': reco['tb_rot'] + reio['tb_rot'],
        'ee_rot_reco': reco['ee_rot'],
        'ee_rot_reio': reio['ee_rot'],
        'bb_rot_reco': reco['bb_rot'],
        'bb_rot_reio': reio['bb_rot'],
        'eb_rot_reco': reco['eb_rot'],
        'eb_rot_reio': reio['eb_rot'],
        'te_rot_reco': reco['te_rot'],
        'te_rot_reio': reio['te_rot'],
        'tb_rot_reco': reco['tb_rot'],
        'tb_rot_reio': reio['tb_rot'],
        'alpha0_reco': reco['alpha0'],
        'alpha0_reio': reio['alpha0'],
        'V_alpha_reco': reco['V_alpha'],
        'V_alpha_reio': reio['V_alpha'],
    }
...
cdef class Class:
    ...
    def compute(self, level=["distortions"]):
        ...
        if "harmonic" in level:
            if harmonic_init(&(self.pr), &(self.ba), &(self.th), &(self.pt), &(self.pm), &(self.fo), &(self.tr), &(self.hr)) == _FAILURE_:
                self.struct_cleanup()
    ...
    def raw_cl(self, lmax=-1, nofail=False):
        ...
        -------
        cl : dict
            Dictionary that contains the unlensed harmonic spectra returned by CLASS,
            including the standard CMB spectra and, when available, the birefringence
            ingredients and reco/reio split spectra needed for an external rotation step.
        """
        ...
        has_flags = [
            ...
            (self.hr.has_aa, self.hr.index_ct_aa, 'aa'),
            (self.hr.has_aa_reco, self.hr.index_ct_aa_reco, 'aa_reco'),
            (self.hr.has_aa_reio, self.hr.index_ct_aa_reio, 'aa_reio'),
            (self.hr.has_at, self.hr.index_ct_at, 'at'),
            (self.hr.has_at_reco, self.hr.index_ct_at_reco, 'at_reco'),
            (self.hr.has_at_reio, self.hr.index_ct_at_reio, 'at_reio'),
            (self.hr.has_ae, self.hr.index_ct_ae, 'ae'),
            (self.hr.has_ae_reco, self.hr.index_ct_ae_reco, 'ae_reco'),
            (self.hr.has_ae_reio, self.hr.index_ct_ae_reio, 'ae_reio'),
            (self.hr.has_ee_reco, self.hr.index_ct_ee_reco, 'ee_reco'),
            (self.hr.has_ee_reio, self.hr.index_ct_ee_reio, 'ee_reio'),
            (self.hr.has_te_reco, self.hr.index_ct_te_reco, 'te_reco'),
            (self.hr.has_te_reio, self.hr.index_ct_te_reio, 'te_reio'),]
        spectra = []
    ...
    def rotation_cl_inputs(self, lmax=-1, nofail=False):
        """
        Return the spectra and isotropic angles needed to rotate the spectra externally.

        This helper packages the birefringence-related harmonic outputs together with
        the sharp-split isotropic angles `alpha0_reco` and `alpha0_reio`, so the
        Wigner-3j rotation can be carried out on the Python side.
        """
        self.compute(["harmonic"])
        cdef int index_th
        cdef int index_peak_reco = -1
        cdef int index_peak_reio = -1
        cdef int last_index = 0
        cdef double g_mid
        cdef double g_prev
        cdef double g_next
        cdef double g_peak_reco = -1.0
        cdef double g_peak_reio = -1.0
        cdef double z_reco
        cdef double z_tab
        cdef double tau
        cdef double chi0_today
        cdef double chi0_reco
        cdef double chi0_reio
        cdef double * pvecback

        cl = self.raw_cl(lmax, nofail)
        ...
        try:
            for index_th in range(self.th.tt_size):
                g_mid = self.th.thermodynamics_table[index_th*self.th.th_size + self.th.index_th_g]
                if g_mid > g_peak_reco:
                    g_peak_reco = g_mid
                    index_peak_reco = index_th

            if index_peak_reco < 0:
                raise CosmoSevereError("Could not identify the recombination visibility peak.")

            z_reco = self.th.z_table[index_peak_reco]

            for index_th in range(1, self.th.tt_size-1):
                z_tab = self.th.z_table[index_th]
                g_prev = self.th.thermodynamics_table[(index_th-1)*self.th.th_size + self.th.index_th_g]
                g_mid = self.th.thermodynamics_table[index_th*self.th.th_size + self.th.index_th_g]
                g_next = self.th.thermodynamics_table[(index_th+1)*self.th.th_size + self.th.index_th_g]

                if z_tab < z_reco and g_mid >= g_prev and g_mid > g_next and g_mid > g_peak_reio:
                    g_peak_reio = g_mid
                    index_peak_reio = index_th

            if index_peak_reio < 0:
                for index_th in range(self.th.tt_size):
                    z_tab = self.th.z_table[index_th]
                    g_mid = self.th.thermodynamics_table[index_th*self.th.th_size + self.th.index_th_g]
                    if z_tab < z_reco and g_mid > g_peak_reio:
                        g_peak_reio = g_mid
                        index_peak_reio = index_th

            if index_peak_reio < 0:
                raise CosmoSevereError("Could not identify the reionization visibility peak.")

            if background_at_tau(&self.ba, self.ba.conformal_age, long_info, inter_normal, &last_index, pvecback) == _FAILURE_:
                raise CosmoSevereError(self.ba.error_message)
            chi0_today = pvecback[self.ba.index_bg_chi0]

            if background_tau_of_z(&self.ba, self.th.z_table[index_peak_reco], &tau) == _FAILURE_:
                raise CosmoSevereError(self.ba.error_message)
            last_index = 0
            if background_at_tau(&self.ba, tau, long_info, inter_normal, &last_index, pvecback) == _FAILURE_:
                raise CosmoSevereError(self.ba.error_message)
            chi0_reco = pvecback[self.ba.index_bg_chi0]

            if background_tau_of_z(&self.ba, self.th.z_table[index_peak_reio], &tau) == _FAILURE_:
                raise CosmoSevereError(self.ba.error_message)
            last_index = 0
            if background_at_tau(&self.ba, tau, long_info, inter_normal, &last_index, pvecback) == _FAILURE_:
                raise CosmoSevereError(self.ba.error_message)
            chi0_reio = pvecback[self.ba.index_bg_chi0]
        finally:
            free(pvecback)
        ...
        rot['alpha0_reco'] = 0.5 * self.pr.lambda_over_f * (chi0_today - chi0_reco)
        rot['alpha0_reio'] = 0.5 * self.pr.lambda_over_f * (chi0_today - chi0_reio)

        return rot

    def rotated_cl(self, lmax=-1, nofail=False):
        """
        Return the birefringence-rotated spectra, computed in Python.

        This applies the same second-order Wigner-3j rotation that the old
        harmonic-space C path used, but now fully on the Python side.
        """
        rot_inputs = self.rotation_cl_inputs(lmax, nofail)
        return rotate_birefringence_cl(rot_inputs)
