 ...
 *
 * The strict tomographic approximation used here samples the relevant
 * source integrands exactly at these two peaks, consistently with the
 * replacement g(eta) -> g_rec delta(eta-eta_rec) + g_reio delta(eta-eta_rei).
 */
...
/**
 * Legacy helper kept only to avoid touching unrelated call sites.
 */
static double transfer_tomo_reco_weight(
  ...
  if ((has_peak_reio == _FALSE_) ||
      (tau_split <= tau_peak_reco) ||
      (tau_peak_reio <= tau_split)) {
    return (tau <= tau_split) ? 1.0 : 0.0;
  }

  sigma_reco = MAX(sigma_reco, 1e-12 * MAX(1.0, tau_peak_reco));
  sigma_reio = MAX(sigma_reio, 1e-12 * MAX(1.0, tau_peak_reio));

  arg_rec = (tau - tau_peak_reco) / sigma_reco;
  arg_reio = (tau - tau_peak_reio) / sigma_reio;

  w_rec = exp(-0.5 * arg_rec * arg_rec);
  w_reio = exp(-0.5 * arg_reio * arg_reio);
  denom = w_rec + w_reio;
 ...
 * onto the nearest sampled perturbation-source time around the relevant
 * visibility peak.
 */
 ...
 * visibility peak. We enforce the delta-time approximation here by
...
/**
 * Store the strict two-delta tomographic source corresponding to the
 * sum of the recombination and reionization contributions. This is
 * used for the full alpha transfer so that it remains consistent with
 * the reco/reio split channels.
 */
 ...
 * split. This is intentionally different from alpha: the delta-time
 * approximation is enforced only for the birefringence source, while
 * the CMB T/E source keeps its broad emission history.
 */
 ...
 * tomographic localization or split. We use this for alpha_total so that
 * the total birefringence channel keeps the original CLASS integration,
 * while only alpha_reco/alpha_reio are pushed toward the sharp-time
 * approximation.
 */
...
int transfer_init(
  ...
  */
  ...
  ptr->tau_tomo_peak_reco = tau_rec;
  ...
  class_call(transfer_visibility_peak_windows(pth,
  ...
  if (ptr->transfer_verbose > 0) {
    if (ptr->has_tomo_peak_reio == _TRUE_) {
      fprintf(stdout,
              "[tomo visibility peaks] reco: thermo_index=%d source_index=%d z=%g tau=%g g=%e width_bins=%d sigma_tau=%e | reio: thermo_index=%d source_index=%d z=%g tau=%g g=%e width_bins=%d sigma_tau=%e | g_reio/g_reco=%e\n",
              index_peak_reco,
              index_tau_peak_reco,
              z_peak_reco,
              ptr->tau_tomo_peak_reco,
              g_peak_reco,
              2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1,
              ptr->tau_tomo_sigma_reco,
              index_peak_reio,
              index_tau_peak_reio,
              z_peak_reio,
              ...
              2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1,
              ptr->tau_tomo_sigma_reio,
              g_peak_reio/g_peak_reco);
    }
    else {
      fprintf(stdout,
              "[tomo visibility peaks] reco: thermo_index=%d source_index=%d z=%g tau=%g g=%e width_bins=%d sigma_tau=%e | reio: none found\n",
              index_peak_reco,
              index_tau_peak_reco,
              z_peak_reco,
              ptr->tau_tomo_peak_reco,
...
int transfer_source_tau_size(
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_source_alpha == _TRUE_) && (index_tt == ptr->index_tt_alpha))
      *tau_size = (ptr->has_tomo_peak_reio == _TRUE_) ? 2 : 1;
...
int transfer_sources(
  ...
  int index_tau_peak_reco_diag = -1;
  int index_tau_peak_reio_diag = -1;
  int width_bins_reco_diag = 0;
  int width_bins_reio_diag = 0;
      ...
      if (is_tomo_temp == _TRUE_) {
        ...
        if (is_tomo_alpha_total == _TRUE_) {
          ...
          class_call(transfer_store_localized_peak_source(ppt,
                                                          ...
                                                          w_trapz,
                                                          &tau_size,
                                                          &index_tau_peak_reco_diag,
                                                          &width_bins_reco_diag),
                     ptr->error_message,
                     ptr->error_message);
        }
        else if ((is_reio_channel == _TRUE_) && (ptr->has_tomo_peak_reio == _TRUE_)) {
          class_call(transfer_store_localized_peak_source(ppt,
                                                          ...
                                                          w_trapz,
                                                          &tau_size,
                                                          &index_tau_peak_reio_diag,
                                                          &width_bins_reio_diag),
                     ptr->error_message,
                     ptr->error_message);
        }
        else {
        ...
        if (is_tomo_alpha_total == _FALSE_) {
          double alpha_full_norm;
          double alpha_reco_norm = 0.0;
          double alpha_reio_norm = 0.0;
          double alpha_split_norm;
          double alpha_rescale = 1.0;
          int tau_size_reco_norm = 0;
          int tau_size_reio_norm = 0;
          int index_tau_tmp = -1;
          int width_tmp = 0;
          int index_tau_norm;
          double alpha_sources_reco_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
          double alpha_tau_reco_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
          double alpha_w_reco_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
          double alpha_sources_reio_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
          double alpha_tau_reio_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
          double alpha_w_reio_norm[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];

          alpha_full_norm = transfer_integrated_full_history_source(ppt,
                                                                    interpolated_sources);
          ...
          alpha_reco_norm = transfer_integrated_source(alpha_sources_reco_norm,
                                                       alpha_w_reco_norm,
                                                       tau_size_reco_norm);
          ...
          if (ptr->has_tomo_peak_reio == _TRUE_) {
            ...
            alpha_reio_norm = transfer_integrated_source(alpha_sources_reio_norm,
                                                         alpha_w_reio_norm,
                                                         tau_size_reio_norm);
          }

          alpha_split_norm = alpha_reco_norm + alpha_reio_norm;

          if (fabs(alpha_split_norm) > 0.0) {
            alpha_rescale = alpha_full_norm / alpha_split_norm;
          }

          for (index_tau_norm = 0; index_tau_norm < tau_size; index_tau_norm++) {
            sources[index_tau_norm] *= alpha_rescale;
          }
        }

        if (ptr->transfer_verbose > 0) {
          static const double diag_k_targets[3] = {1.0e-5, 5.0e-5, 1.0e-4};
          static int diag_k_seen[3] = {0, 0, 0};
          int index_diag_k;

          for (index_diag_k = 0; index_diag_k < 3; index_diag_k++) {
            if ((diag_k_seen[index_diag_k] == 0) &&
                (fabs(k/diag_k_targets[index_diag_k] - 1.0) < 0.15)) {
              double alpha_total_sum = 0.0;
              double alpha_reco_sum = 0.0;
              double alpha_reio_sum = 0.0;
              double alpha_split_sum;
              double alpha_rescale_diag = 1.0;
              double mismatch;
              int tau_size_total_diag = 0;
              int tau_size_reco_diag = 0;
              int tau_size_reio_diag = 0;
              int index_tau_reco_tmp = -1;
              int index_tau_reio_tmp = -1;
              int width_reco_tmp = 0;
              int width_reio_tmp = 0;
              double diag_sources_reco[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              double diag_tau0_minus_tau_reco[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              double diag_w_trapz_reco[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              double diag_sources_reio[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              double diag_tau0_minus_tau_reio[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              double diag_w_trapz_reio[(2*_TOMO_DELTA_PEAK_HALF_WIDTH_+1)];
              ...
              tau_size_total_diag = ppt->tau_size;
              alpha_total_sum = transfer_integrated_full_history_source(ppt,
                                                                        interpolated_sources);

              alpha_reco_sum = transfer_integrated_source(diag_sources_reco,
                                                          diag_w_trapz_reco,
                                                          tau_size_reco_diag);

              alpha_reio_sum = transfer_integrated_source(diag_sources_reio,
                                                          diag_w_trapz_reio,
                                                          tau_size_reio_diag);

              alpha_split_sum = alpha_reco_sum + alpha_reio_sum;
              if (fabs(alpha_split_sum) > 0.0) {
                alpha_rescale_diag = alpha_total_sum / alpha_split_sum;
                alpha_reco_sum *= alpha_rescale_diag;
                alpha_reio_sum *= alpha_rescale_diag;
                alpha_split_sum = alpha_reco_sum + alpha_reio_sum;
              }

              if (fabs(alpha_total_sum) > 0.0) {
                mismatch = fabs(alpha_split_sum - alpha_total_sum) / fabs(alpha_total_sum);
              }
              else {
                mismatch = fabs(alpha_split_sum - alpha_total_sum);
              }

              fprintf(stdout,
                      "[tomo alpha source] k_target=%e matched_k=%e | reco: source_index=%d width_bins=%d tau=%g | reio: source_index=%d width_bins=%d tau=%g | alpha_total(full)=%e alpha_reco+alpha_reio=%e rescale=%e rel_mismatch=%e\n",
                      diag_k_targets[index_diag_k],
                      k,
                      index_tau_reco_tmp,
                      width_reco_tmp,
                      ptr->tau_tomo_peak_reco,
