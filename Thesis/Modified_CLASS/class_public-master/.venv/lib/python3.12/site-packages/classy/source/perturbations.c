...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_alpha == _TRUE_) {
      ...
      /*
       * Tomographic birefringence source.
       ...
       *
       * In the 2023 tomographic treatment, the anisotropic angle at a
       * given emission epoch is proportional to delta_chi evaluated at
       * that epoch, not to g * delta_chi. The transfer module then
       * localizes this source at the recombination and reionization
       * visibility peaks.
       */
      ...
      _set_source_(ppt->index_tp_alpha) =
        (-0.5 * ppr->lambda_over_f) * delta_chi_for_alpha;
    }
    /* delta_dr */
