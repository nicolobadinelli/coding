...
int output_open_cl_file(
  ...
  if (pop->write_header == _TRUE_) {
    ...
    if (pop->output_format == class_format) {
      ...
      class_fprintf_columntitle(*clfile,"alphaalpha",phr->has_aa,colnum);
      class_fprintf_columntitle(*clfile,"alphaT",phr->has_at,colnum);
      class_fprintf_columntitle(*clfile,"alphaE",phr->has_ae,colnum);
      class_fprintf_columntitle(*clfile,"alphaphi",phr->has_ap,colnum);
      ...
      class_fprintf_columntitle(*clfile,"Ad",phr->has_ap,colnum);
    ...
    if (phr->has_ad == _TRUE_){
      for (index_d1=0; index_d1<phr->d_size; index_d1++){
        sprintf(tmp,"alpha-dens[%d]",index_d1+1);
        class_fprintf_columntitle(*clfile,tmp,_TRUE_,colnum);
      }
    }
...
int output_one_line_of_cl(
  ...
  if (pop->output_format == camb_format) {
    ...
    class_fprintf_double(clfile, sqrt(l*(l+1))*factor*cl[phr->index_ct_ap], phr->has_ap);
