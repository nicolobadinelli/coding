...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (compute_sigma8 == _TRUE_) {
    ...
    pt.has_cl_cmb_birefringence = _FALSE_;
...
int input_read_parameters_general(struct file_content * pfc,
  ...
  char * options_output[36] =  {"tCl","pCl","lCl","nCl","dCl","sCl","aCl","mPk","mTk","dTk","vTk","sd",
                                "TCl","PCl","LCl","NCl","DCl","SCl","ACl","MPk","MTk","DTk","VTk","Sd",
                                "TCL","PCL","LCL","NCL","DCL","SCL","ACL","MPK","MTK","DTK","VTK","SD"};
  ...
  if (flag1 == _TRUE_) {
    ...
    if ((strstr(string1,"aCl") != NULL) || (strstr(string1,"ACl") != NULL) || (strstr(string1,"ACL") != NULL)) {
      ppt->has_cl_cmb_birefringence = _TRUE_;
      ppt->has_perturbations = _TRUE_;
      ppt->has_cls = _TRUE_;
    }
  ...
  if (ppt->has_perturbations == _TRUE_) {
    ...
    if (ppt->has_vectors == _TRUE_){
      ...
      class_test(ppt->has_birefringence == _TRUE_,
                 errmsg,
                 "The current version of CLASS does not allow cosmic birefringence if vector modes are present.");
    }
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if (flag1 == _TRUE_) {
        ...
        if ((strstr(string1,"bi") != NULL) || (strstr(string1,"BI") != NULL)){
          ppt->has_bi=_TRUE_;
          if (ppt->has_cl_cmb_birefringence == _TRUE_){
            class_stop(errmsg,"The current model of anisotropic birefringence is implemented only for adiabatic initial conditions.");
          }
        }
      ...
      /* Test */
      class_test(ppt->has_cl_cmb_birefringence == _TRUE_,
                 errmsg,
                 "Inconsistency: you want C_l's for anisotropic birefringence, but no scalar modes\n");
  ...
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"newtonian") != NULL) || (strstr(string1,"Newtonian") != NULL) || (strstr(string1,"new") != NULL)) {
      ppt->gauge = newtonian;
      /*if (ppt->has_cl_cmb_birefringence == _TRUE_) {
        class_stop(errmsg,"The current model of anisotropic birefringence is not implemented in the Newtonian gauge.");
      }*/
    }
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 8) Dark energy
      ...
      fluid), Omega0_scf (scalar field), Omega0_axf (axion field) */
  /* Read */
  ...
  class_call(parser_read_double(pfc,"Omega_scf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_string(pfc,"Axion_field",&string4,&flag4,errmsg),
               errmsg,
               errmsg);
  /* Complete set of parameters */
  if ((flag4 == _TRUE_) && (string_begins_with(string4,'y') || string_begins_with(string4,'Y'))) {
      pba->Omega0_axf = 1;
  }
  /* Test */
  ...
  class_test((     ((flag1 == _TRUE_) && (param1 != 0.))
                ...
                && ((flag4 == _TRUE_) && (pba->Omega0_axf != 0.)),
             errmsg,
             "This version of CLASS does not allows for multi-components of dark energy if an axion field is present.");
  /* Complete set of parameters
  ...
  /** 8.c) If Omega axion field (AXF) is different from 0 */
  if (pba->Omega0_axf != 0.){


    /** 8.c.1) Determine the kind of evolution for the axion field */
      class_call(parser_read_string(pfc,"axion_evolution",&string1,&flag1,errmsg),
               errmsg,
               errmsg);
      /* Complete set of parameters */
      if (flag1 == _TRUE_) {
        if ((strstr(string1,"NQF") != NULL) || (strstr(string1,"nqf") != NULL)) {
          pba->axion_evolution = NQF;
        }
        else if ((strstr(string1,"PDE") != NULL) || (strstr(string1,"pde") != NULL)) {
          pba->axion_evolution = PDE;
        }
        else {
          class_stop(errmsg,"incomprehensible input '%s' for the field 'axion_evolution'",string1);
        } 
      }

    /** 8.c.2) Additional AXF parameters */
    /* Read */
    class_call(parser_read_list_of_doubles(pfc,
                                           "axf_parameters",
                                           &(pba->axf_parameters_size),
                                           &(pba->axf_parameters),
                                           &flag2,
                                           errmsg),
               errmsg,errmsg);

    /** 8.c.3) Complete set of parameters */
      if (flag2 == _TRUE_) {
        pba->chi_ini_axf = pba->axf_parameters[pba->axf_parameters_size-2];
        pba->chi_prime_ini_axf = pba->axf_parameters[pba->axf_parameters_size-1];
      }
    class_test(pba->axf_parameters_size != 3,errmsg,"You need at least 3 parameters for the axion field: mass_chi, chi_ini, chi_prime_ini.");

    /** 8.c.3) AXF tuning parameter */
    /* Read */
 //   class_read_int("axf_tuning_index",pba->axf_tuning_index);
    /* Test */
 /*   class_test(pba->axf_tuning_index >= pba->axf_parameters_size,
               errmsg,
               "Tuning index 'axf_tuning_index' (%d) is larger than the number of entries (%d) in 'axf_parameters'.",
               pba->axf_tuning_index,pba->axf_parameters_size);                                        */

    /** 8.c.4) Shooting parameter */
    /* Read */
//    class_read_double("axf_shooting_parameter",pba->axf_parameters[pba->axf_tuning_index]);
    /* Complete set of parameters */
/*    axf_mass = pba->axf_parameters[0];
    if ((axf_mass < 0)&&(pba->background_verbose>1)){
      printf("'mass_chi' = %e < 0 is not acceptable: the axion mass can only be positive.",axf_mass);
    }                                                                                                  */
...
int input_read_parameters_spectra(struct file_content * pfc,
  ...
  if (ppt->has_cls == _TRUE_) {
    if (ppt->has_scalars == _TRUE_) {
      if ((ppt->has_cl_cmb_temperature == _TRUE_) || (ppt->has_cl_cmb_polarization == _TRUE_) 
        || (ppt->has_cl_cmb_birefringence == _TRUE_) || (ppt->has_cl_cmb_lensing_potential == _TRUE_)){
  ...
  /** 4) Cosmic birefringence induced by the axion field (AXF) */
  /* Read */
  class_call(parser_read_string(pfc,"birefringence",&string1,&flag1,errmsg),
               errmsg,
               errmsg);
  /* Complete set of parameters */
  if ((flag1 == _TRUE_) && (string_begins_with(string1,'y') || string_begins_with(string1,'Y'))) {
      ppt->has_birefringence = _TRUE_;
  }
  class_call(parser_read_string(pfc,"instant_emission",&string2,&flag2,errmsg),
               errmsg,
               errmsg);
  /* Complete set of parameters */
  if ((flag2 == _TRUE_) && (string_begins_with(string2,'y') || string_begins_with(string2,'Y'))) {
      ppt->instant_emission = _TRUE_;
  }

  /** 4.a) If cosmic birefringence is switched on */
  /* Read */
  if (ppt->has_birefringence == _TRUE_) {

    class_call(parser_read_double(pfc,"axf_g",&param3,&flag3,errmsg),
               errmsg,
               errmsg);

    /* Complete set of parameters */
    if (flag3 == _TRUE_) {
        pba->axf_g = param3*2.436*pow(10,18);
      }

    /* Test */
    class_test(pba->Omega0_axf == 0,
               errmsg,
               "You asked for cosmic birefringence: the presence of an axion field is required.");
    class_test(pba->axf_g == 0,
               errmsg,
               "You asked for cosmic birefringence: a non-zero value of 'axf_g' is required.");
    class_test(ppt->has_vectors == _TRUE_,
               errmsg,
               "The current version of CLASS does not allow for cosmic birefringence if vector modes are present.");
    class_test(ppt->has_tensors == _TRUE_,
               errmsg,
               "The current version of CLASS does not allow for cosmic birefringence if tensor modes are present.");
  }

  /** 4.b) If cosmic birefringence is switched off */
  /* Test */
  else {
    class_test(ppt->has_cl_cmb_birefringence == _TRUE_,
               errmsg,
               "You asked for the Cl's of cosmic birefringence: this requires setting 'birefringence' equal to 'yes'.")
...
int input_read_parameters_lensing(struct file_content * pfc,
  ...
  /* Complete set of parameters */
  if ((flag1 == _TRUE_) && (string_begins_with(string1,'y') || string_begins_with(string1,'Y'))){
    if ((ppt->has_scalars == _TRUE_) && ((ppt->has_cl_cmb_temperature == _TRUE_) || (ppt->has_cl_cmb_polarization == _TRUE_) 
          || (ppt->has_cl_cmb_birefringence == _TRUE_)) && (ppt->has_cl_cmb_lensing_potential == _TRUE_)){
...
int input_default_params(struct background *pba,
  ...
  ppt->has_cl_cmb_birefringence = _FALSE_;
  ppt->instant_emission = _FALSE_;
  ...
  /** 9.c) Omega axion field */
  /** 9.c.1) Axion field evolution */
  pba->axion_evolution = NQF;
  /** 9.c.2) Potential parameters and initial conditions */
  pba->axf_parameters = NULL;
  pba->axf_parameters_size = 0;
  /** 9.c.3) Tuning parameter */
//pba->axf_tuning_index = 0; 
