...
int background_functions(
  ...
  double rho_m;
  /* remove extra dark energy density due to the presence of an axion field */
  double remove_exde;
  /* remove extra dark matter density due to the presence of an axion field */
  double remove_exdm;
  ...
  /* Since we only know a_prime_over_a after we have rho_tot,
     ...
     Note: The axion field contribution will be not add to the Friedmann Equation!*/
  double dp_dloga;
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    /* Axion field */
  if (pba->has_axf == _TRUE_) {
    chi = pvecback_B[pba->index_bi_chi_axf];
    chi_prime = pvecback_B[pba->index_bi_chi_prime_axf];
    pvecback[pba->index_bg_chi_axf] = chi; // value of the axion field chi
    pvecback[pba->index_bg_chi_prime_axf] = chi_prime; // value of the axion field chi derivative wrt conformal time
    pvecback[pba->index_bg_V_axf] = V_axf(pba,chi); //V_axf(pba,chi); //write here potential as function of chi
    pvecback[pba->index_bg_dV_axf] = dV_axf(pba,chi); // dV_axf(pba,chi); //potential' as function of chi
    pvecback[pba->index_bg_ddV_axf] = ddV_axf(pba,chi); // ddV_axf(pba,chi); //potential'' as function of chi
    pvecback[pba->index_bg_rho_axf] = (chi_prime*chi_prime/(2*a*a) + V_axf(pba,chi))/3.; // energy of the axion field. The field units are set automatically by setting the initial conditions
    pvecback[pba->index_bg_p_axf] = (chi_prime*chi_prime/(2*a*a) - V_axf(pba,chi))/3.; // pressure of the axion field
    pvecback[pba->index_bg_w_axf] = (chi_prime*chi_prime - 2*a*a*V_axf(pba,chi))/(chi_prime*chi_prime + 2*a*a*V_axf(pba,chi)); // equation of state for the axion field
    pvecback[pba->index_bg_beta_axf] = 0.5*pba->axf_g*(pba->chi0 - chi); // isotropic birefringence angle according to the conventions of 2203.08560
    
    /* Evolution scheme */

    switch (pba->axion_evolution) {
      case NQF: // Negligible Quintessence Field
        /*class_test(pvecback[pba->index_bg_rho_axf] > 0.2*rho_tot,
                   pba->error_message,
                   "A Negligible Quintessence Field (NQF) must have a negligible energy density.");*/
        rho_tot += 0;
        p_tot += 0;
        remove_exde = 0.;
        remove_exdm = 0.;
      break;
      case PDE: // Pure Dark Energy
        class_test(pvecback[pba->index_bg_w_axf] > - 1/3,
                   pba->error_message,
                   "Pure Dark energy must (PDE) have equation of state less than - 1/3.");
        class_test(pvecback[pba->index_bg_rho_axf] > pba->Omega0_lambda * pow(pba->H0,2),
                   pba->error_message,
                   "The axion field dark energy density cannot be larger than the input value for the dark energy density.");
        rho_tot += pvecback[pba->index_bg_rho_axf];
        p_tot += pvecback[pba->index_bg_p_axf];
        remove_exde = 1.;
        remove_exdm = 0.;
      break;
  }
  ...
  if (pba->has_cdm == _TRUE_) {
    pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3) - remove_exdm*pvecback[pba->index_bg_rho_axf];
    rho_tot += pvecback[pba->index_bg_rho_cdm];
    p_tot -= remove_exdm*pvecback[pba->index_bg_p_axf];
  ...
  if (pba->has_lambda == _TRUE_) {
    pvecback[pba->index_bg_rho_lambda] = pba->Omega0_lambda * pow(pba->H0,2) - remove_exde*pvecback[pba->index_bg_rho_axf];
    rho_tot += pvecback[pba->index_bg_rho_lambda];
    p_tot -= (pba->Omega0_lambda * pow(pba->H0,2) + remove_exde*pvecback[pba->index_bg_p_axf]);
  }
  ...
  pvecback[pba->index_bg_p_tot_prime] = a*pvecback[pba->index_bg_H]*dp_dloga;
  if (pba->has_axf == _TRUE_) {
    pvecback[pba->index_bg_p_prime_axf] = pvecback[pba->index_bg_chi_prime_axf]*
      (-pvecback[pba->index_bg_chi_prime_axf]*pvecback[pba->index_bg_H]/a-2./3.*pvecback[pba->index_bg_dV_axf]);
    pvecback[pba->index_bg_p_tot_prime] += pvecback[pba->index_bg_p_prime_axf];
  }
...
int background_free_input(
  ...
  if (pba->Omega0_axf != 0.) {
    if (pba->axf_parameters != NULL)
      free(pba->axf_parameters);
  }
...
int background_indices(
  ...
  /* - indices for axion field */
  class_define_index(pba->index_bg_chi_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_chi_prime_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_V_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_dV_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_ddV_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_rho_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_p_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_p_prime_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_w_axf,pba->has_axf,index_bg,1);
  class_define_index(pba->index_bg_beta_axf,pba->has_axf,index_bg,1);
  ...
  /* -> axion field and its derivative wrt conformal time */
  class_define_index(pba->index_bi_chi_axf,pba->has_axf,index_bi,1);
  class_define_index(pba->index_bi_chi_prime_axf,pba->has_axf,index_bi,1);
...
int background_solve(
  ...
  int index_loga, index_scf, index_axf;
  ...
  /* -> value of the axion field in units of the reduced Planck mass */
  pba->chi0 = pvecback_integration[pba->index_bi_chi_axf];
  /* -> age in Gyears */
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    /** - Fix initial value of \f$ \chi, \chi' \f$
   */
  if (pba->has_axf == _TRUE_) {
    pvecback_integration[pba->index_bi_chi_axf] = pba->chi_ini_axf;
    pvecback_integration[pba->index_bi_chi_prime_axf] = pba->chi_prime_ini_axf;
    class_test(!isfinite(pvecback_integration[pba->index_bi_chi_axf]) ||
               !isfinite(pvecback_integration[pba->index_bi_chi_axf]),
               pba->error_message,
               "initial chi = %e chi_prime = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_chi_axf],
               pvecback_integration[pba->index_bi_chi_axf]);
  }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_axf",pba->has_axf);
  class_store_columntitle(titles,"(.)p_axf",pba->has_axf);
  class_store_columntitle(titles,"(.)p_prime_axf",pba->has_axf);
  class_store_columntitle(titles,"chi_axf [m_Pl]",pba->has_axf);
  class_store_columntitle(titles,"chi'_axf [m_Pl/Mpc]",pba->has_axf);
  class_store_columntitle(titles,"V_axf [m_Pl^2/Mpc^2]",pba->has_axf);
  class_store_columntitle(titles,"V'_axf [m_Pl/Mpc^2]",pba->has_axf);
  class_store_columntitle(titles,"V''_axf [1/Mpc^2]",pba->has_axf);
  class_store_columntitle(titles,"w_axf",pba->has_axf);
  class_store_columntitle(titles,"beta_axf [rad]",pba->has_axf);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_prime_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_chi_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_chi_prime_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_V_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dV_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_ddV_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_axf],pba->has_axf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_beta_axf],pba->has_axf,storeidx);
...
int background_derivs(
  ...
  if (pba->has_axf == _TRUE_) {
    /** - Axion field equation: \f$ \chi'' + 2 a H \chi' + a^2 dV = 0 \f$  (note H is wrt cosmological time)
        written as \f$ d\chi/dlna = chi' / (aH) \f$ and \f$ d\chi'/dlna = -2*chi' - (a/H) dV \f$ */
    dy[pba->index_bi_chi_axf] = y[pba->index_bi_chi_prime_axf]/a/H;
    dy[pba->index_bi_chi_prime_axf] = - 2*y[pba->index_bi_chi_prime_axf] - a*dV_axf(pba,y[pba->index_bi_chi_axf])/H ;
  }
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if ((pba->has_lambda == _TRUE_) || (pba->has_fld == _TRUE_) || (pba->has_scf == _TRUE_) 
        || (pba->has_axf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
...
/**
 * Axion field potential and its derivatives with respect to the field _axf:
 * 
 * - \f$ V = V_{m_{axf}} \f$ with \f$ V_m = 0.5*mass^2*chi^2 \f$ (Nakatsuka et al. model: 2203.08560)
 * - \f$ V = V_{n_{axf}} \f$ with \f$ V_n = mass^2*(1-cos(chi))^2 \f$ (Capparelli et al. model: 1909.04621)
 * 
 * The units of chi, tau in the derivatives and the potential V are the following:
 * - chi is given in units of the reduced Planck mass \f$ m_{pl} = (8 \pi G)^{(-1/2)}\f$
 * - tau in the derivative is given in units of Mpc.
 * - the potential \f$ V(\chi) \f$ is given in units of \f$ m_{pl}^2/Mpc^2 \f$.
 * With this convention, we have
 * \f$ \rho^{class} = (8 \pi G)/3 \rho^{physical} = 1/(3 m_{pl}^2) \rho^{physical} = 1/3 * [ 1/(2a^2) (\chi')^2 + V(\chi) ] \f$
 and \f$ \rho^{class} \f$ has the proper dimension \f$ Mpc^-2 \f$.
*/

double V_m_axf(struct background *pba,
               double chi
               ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];

  return  0.5*pow(axf_mass,2)*pow(chi,2);
}

double V_n_axf(struct background *pba,
               double chi
               ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];
  double n_axf = 3.0;

  return  pow(axf_mass,2)*pow(1-cos(0.199*chi),n_axf)/pow(0.199,2);
}

double dV_m_axf(struct background *pba,
                double chi
                ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];

  return  pow(axf_mass,2)*chi;
}

double dV_n_axf(struct background *pba,
                double chi
                ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];
  double n_axf = 3.0;

  return  pow(axf_mass,2)*n_axf*sin(0.199*chi)*pow(1-cos(0.199*chi),n_axf-1)/0.199;
}

double ddV_m_axf(struct background *pba,
                 double chi
                 ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];

  return pow(axf_mass,2);
}

double ddV_n_axf(struct background *pba,
                 double chi
                 ) {
  double axf_mass = 1.5637277617317205e29*pba->axf_parameters[0];
  double n_axf = 3.0;

  return pow(axf_mass,2)*n_axf*pow(1-cos(0.199*chi),n_axf-2)*((n_axf-1)*pow(sin(0.199*chi),2)-pow(cos(0.199*chi),2)+cos(0.199*chi));
}

/** Finally we can obtain the overall potential \f$ V = V_m \f$ or \f$ V = V_n \f$
 */

double V_axf(
             struct background *pba,
             double chi) {
  return  V_m_axf(pba,chi);
  //return  V_n_axf(pba,chi);
}

double dV_axf(
              struct background *pba,
              double chi) {
  return dV_m_axf(pba,chi);
  //return dV_n_axf(pba,chi);
}

double ddV_axf(
               struct background *pba,
               double chi) {
  return ddV_m_axf(pba,chi);
  //return ddV_n_axf(pba,chi);
}