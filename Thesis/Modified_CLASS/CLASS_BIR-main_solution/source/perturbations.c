    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_chi_axf],ppt->has_source_delta_chi_axf,storeidx);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"delta_chi_axf",pba->has_axf);
    }
    if (ppt->has_velocity_transfers == _TRUE_) {
...
int perturbations_indices(
  ...
  ppt->has_source_delta_chi_axf = _FALSE_;
  ...
  class_define_index(ppt->index_tp_e,ppt->has_source_p,index_type,1);
  class_define_index(ppt->index_tp_b,ppt->has_source_p,index_type,1);
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_axf == _TRUE_)
          ppt->has_source_delta_chi_axf = _TRUE_;
        if (pba->has_ur == _TRUE_)
      ...
      class_define_index(ppt->index_tp_a,          ppt->has_source_a,         index_type,1);
      ...
      class_define_index(ppt->index_tp_delta_chi_axf,ppt->has_source_delta_chi_axf, index_type,1);
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Axion field axf */
      class_store_columntitle(ppt->scalar_titles, "delta_chi_axf", pba->has_axf);
      /** Fluid */
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_chi_axf,pba->has_axf,index_pt,1); /* axion field density */
    class_define_index(ppv->index_pt_chi_prime_axf,pba->has_axf,index_pt,1); /* axion field velocity */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_axf == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_chi_axf] =
          ppw->pv->y[ppw->pv->index_pt_chi_axf];

        ppv->y[ppv->index_pt_chi_prime_axf] =
          ppw->pv->y[ppw->pv->index_pt_chi_prime_axf];
      }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_axf == _TRUE_) {
        ...
        ppw->pv->y[ppw->pv->index_pt_chi_axf] = 0.;
        ppw->pv->y[ppw->pv->index_pt_chi_prime_axf] = 0.;
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_axf == _TRUE_) {
        ...
        ppw->pv->y[ppw->pv->index_pt_chi_axf] += alpha*ppw->pvecback[pba->index_bg_chi_prime_axf];
        ppw->pv->y[ppw->pv->index_pt_chi_prime_axf] +=
          (-2.*a_prime_over_a*alpha*ppw->pvecback[pba->index_bg_chi_prime_axf]
           -a*a* dV_axf(pba,ppw->pvecback[pba->index_bg_chi_axf])*alpha
           +ppw->pvecback[pba->index_bg_chi_prime_axf]*alpha_prime);
      }
  ...
  if (_scalars_) {
    ...
    */
    if (pba->has_scf == _TRUE_) {
     ...
     /* axion field contribution. */
    if (pba->has_axf == _TRUE_) {

      if (ppt->gauge == newtonian){
        delta_chi_axf = y[ppw->pv->index_pt_chi_axf];
      }

      else{
        delta_chi_axf =  y[ppw->pv->index_pt_chi_axf]; //CHECK THE GAUGE
      }
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_p == _TRUE_) {
      ...
      /* Differently from the standard CLASS code, we split E and B modes according to 1410.1799 */
 
     //  _set_source_(ppt->index_tp_p) = sqrt(6.) * g * P;
        if (ppt->has_birefringence == _TRUE_) {
         _set_source_(ppt->index_tp_e) = sqrt(6.) * g * P; // * cos(2. * pvecback[pba->index_bg_beta_axf]);
         _set_source_(ppt->index_tp_b) = sqrt(6.) * g * P * 0; // * sin(2. * pvecback[pba->index_bg_beta_axf]);
       }

       else {
         _set_source_(ppt->index_tp_e) = sqrt(6.) * g * P;
         _set_source_(ppt->index_tp_b) = sqrt(6.) * g * P;
       }

    }

    /* anisotropic birefringence */
    if (ppt->has_source_a == _TRUE_) {

      /* newtonian gauge */

      if (ppt->gauge == newtonian) {
          _set_source_(ppt->index_tp_a) = - g * 0.5 * pba->axf_g * y[ppw->pv->index_pt_chi_axf];
      ...
      if (ppt->gauge == synchronous) {
           _set_source_(ppt->index_tp_a) = - g * 0.5  * pba->axf_g * (y[ppw->pv->index_pt_chi_axf]
                                             + pvecmetric[ppw->index_mt_alpha]*pvecback[pba->index_bg_chi_prime_axf]); 
      }
    ...
    /* delta_chi_axf */
    if (ppt->has_source_delta_chi_axf == _TRUE_) {
      if (ppt->gauge == synchronous){
        delta_chi_axf = y[ppw->pv->index_pt_chi_prime_axf];  //CHECK THE GAUGE
      }
      else{
        delta_chi_axf = y[ppw->pv->index_pt_chi_prime_axf];
      }
      _set_source_(ppt->index_tp_delta_chi_axf) = delta_chi_axf;
    }
  ...
  if (_tensors_) {
    ...
    /* tensor polarization */
    /* Differently from the standard CLASS code, we split E and B modes according to 1410.1799 */
    /* TODO: implement cosmic birefringence */
    if (ppt->has_source_p == _TRUE_) {
  ...
  //  _set_source_(ppt->index_tp_p) = sqrt(6.) * pvecthermo[pth->index_th_g] * P; 
      _set_source_(ppt->index_tp_e) = sqrt(6.) * pvecthermo[pth->index_th_g] * P;
      _set_source_(ppt->index_tp_b) = sqrt(6.) * pvecthermo[pth->index_th_g] * P;
...
int perturbations_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_axf == _TRUE_){
      if (ppt->gauge == synchronous){
        delta_chi_axf =  y[ppw->pv->index_pt_chi_axf]; //FIX THE GAUGE AGAIN!
      }
      else{
        delta_chi_axf =  y[ppw->pv->index_pt_chi_axf];
      }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_axf == _TRUE_) {
        delta_chi_axf += pvecmetric[ppw->index_mt_alpha_prime]*pvecback[pba->index_bg_chi_prime_axf];
      }
    ...
    /* Axion field axf */
    class_store_double(dataptr, delta_chi_axf, pba->has_axf, storeidx);
    /* Fluid */
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_axf == _TRUE_) {
      ...
      dy[pv->index_pt_chi_axf] = y[pv->index_pt_chi_prime_axf];

      if (ppt->gauge == synchronous) {

      dy[pv->index_pt_chi_prime_axf] =  - 2.*a_prime_over_a*y[pv->index_pt_chi_prime_axf]
        - 0.5*pvecmetric[ppw->index_mt_h_prime]*pvecback[pba->index_bg_chi_prime_axf] 
        - (k2 + a2*pvecback[pba->index_bg_ddV_axf])*y[pv->index_pt_chi_axf]; 
        ...
        dy[pv->index_pt_chi_prime_axf] =  - 2.*a_prime_over_a*y[pv->index_pt_chi_prime_axf]
        + 4.*pvecmetric[ppw->index_mt_phi_prime]*pvecback[pba->index_bg_chi_prime_axf] 
        - (k2 + a2*pvecback[pba->index_bg_ddV_axf])*y[pv->index_pt_chi_axf]
        - 2*a2*pvecback[pba->index_bg_dV_axf]*pvecmetric[ppw->index_mt_psi]; 
