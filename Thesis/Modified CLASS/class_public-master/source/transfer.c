  ...
  for (index_md = 0; index_md < ptr->md_size; index_md++) {
    ...
    for (index_tt=0; index_tt<ptr->tt_size[index_md]; index_tt++) {
      ...
      if (_scalars_) {
        ...
        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t0_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t0_reco;

        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t1_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t1_reco;

        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t2_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t2_reco;

        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t0_reio))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t0_reio;

        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t1_reio))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_t1_reio;

        if ((ppt->has_cl_cmb_temperature == _TRUE_) && (index_tt == ptr->index_tt_t2_reio))
        ...
        if ((ppt->has_cl_cmb_polarization == _TRUE_) && (index_tt == ptr->index_tt_e_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_p_reco;

        if ((ppt->has_cl_cmb_polarization == _TRUE_) && (index_tt == ptr->index_tt_e_reio))
        ...
        if ((ppt->has_source_alpha == _TRUE_) && (index_tt == ptr->index_tt_alpha_reco))
          tp_of_tt[index_md][index_tt]=ppt->index_tp_alpha_reco;
...
int transfer_sources(
  ...
  /** - case where we do not need to redefine */
    ...
    /* plain copy from input array to output array.
       The alpha / alpha_reco / alpha_reio split is done upstream
       in perturbations.c through distinct source channels, so no extra
       windowing must be applied here. */
    for (index_tau=0; index_tau < ppt->tau_size; index_tau++) {
