...
int background_functions(
  ...
  /* Spectator chi field */
  if ((pba->has_chi == _TRUE_) && (return_format != short_info)) {
    double chi0;
    double chi0_prime;

    chi0       = pvecback_B[pba->index_bi_chi0];
    chi0_prime = pvecback_B[pba->index_bi_chi0_prime];

    pvecback[pba->index_bg_chi0]       = chi0;
    pvecback[pba->index_bg_chi0_prime] = chi0_prime;

    pvecback[pba->index_bg_V_chi]   = V_chi(pba, chi0);
    pvecback[pba->index_bg_dV_chi]  = dV_chi(pba, chi0);
    pvecback[pba->index_bg_ddV_chi] = ddV_chi(pba, chi0);
  }
...
int background_indices(
  ...
  class_define_index(pba->index_bg_chi0,pba->has_chi,index_bg,1);
  class_define_index(pba->index_bg_chi0_prime,pba->has_chi,index_bg,1);
  class_define_index(pba->index_bg_V_chi,pba->has_chi,index_bg,1);
  class_define_index(pba->index_bg_dV_chi,pba->has_chi,index_bg,1);
  class_define_index(pba->index_bg_ddV_chi,pba->has_chi,index_bg,1);
  ...
  /* -> spectator chi field and its derivative wrt conformal time */
  class_define_index(pba->index_bi_chi0,pba->has_chi,index_bi,1);
  class_define_index(pba->index_bi_chi0_prime,pba->has_chi,index_bi,1);
...
int background_initial_conditions(
  ...
  a = ppr->a_ini_over_a_today_default;
    ...
    /* spectator chi initial conditions */
  if (pba->has_chi ==_TRUE_){
    pvecback_integration[pba->index_bi_chi0] = pba->chi_ini; 
    pvecback_integration[pba->index_bi_chi0_prime] = pba->chi_prime_ini;
  }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"chi_0",pba->has_chi);
  class_store_columntitle(titles,"chi_0_prime",pba->has_chi);
  class_store_columntitle(titles,"V_chi",pba->has_chi);
  class_store_columntitle(titles,"dV_chi",pba->has_chi);
  class_store_columntitle(titles,"ddV_chi",pba->has_chi);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    if (pba->has_chi == _TRUE_){
      class_store_double(dataptr,pvecback[pba->index_bg_chi0],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_chi0_prime],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_V_chi],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_dV_chi],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_ddV_chi],_TRUE_,storeidx);
    }
  }
...
if (pba->has_chi == _TRUE_) {
  dy[pba->index_bi_chi0] = y[pba->index_bi_chi0_prime]/(a*H);
  dy[pba->index_bi_chi0_prime] =
    -2.0*y[pba->index_bi_chi0_prime] - a*dV_chi(pba, y[pba->index_bi_chi0]) / H;
}
  return _SUCCESS_;
