...
int output_open_cl_file(
  ...
  if (pop->write_header == _TRUE_) {
    ...
    if (pop->output_format == class_format) {
      ...
      class_fprintf_columntitle(*clfile,"TE_rot",phr->has_te_rot,colnum);
      class_fprintf_columntitle(*clfile,"TB_rot",phr->has_tb_rot,colnum);
      class_fprintf_columntitle(*clfile,"EE_rot",phr->has_ee_rot,colnum);
      class_fprintf_columntitle(*clfile,"BB_rot",phr->has_bb_rot,colnum);
      class_fprintf_columntitle(*clfile,"EB_rot",phr->has_eb_rot,colnum);
    }
    else if (pop->output_format == camb_format) {
      class_fprintf_columntitle(*clfile,"TT",phr->has_tt,colnum);
      class_fprintf_columntitle(*clfile,"EE",phr->has_ee,colnum);
...
int output_one_line_of_cl(
  ...
  if (pop->output_format == camb_format) {
    ...
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[phr->index_ct_te_rot], phr->has_te_rot);
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[phr->index_ct_tb_rot], phr->has_tb_rot);
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[phr->index_ct_ee_rot], phr->has_ee_rot);
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[phr->index_ct_bb_rot], phr->has_bb_rot);
    class_fprintf_double(clfile, factor*pow(pba->T_cmb*1.e6,2)*cl[phr->index_ct_eb_rot], phr->has_eb_rot);
    index_ct_rest = 0;
