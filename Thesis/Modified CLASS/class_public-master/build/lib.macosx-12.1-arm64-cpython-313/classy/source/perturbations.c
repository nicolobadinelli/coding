...
int perturbations_output_data_at_z(
  ...
  /* if 0 <= z_pk <= z_max_pk, interpolation needed, */
    ...
    class_test(ppt->ln_tau == NULL,
              "ppt->ln_tau is NULL in perturbations_output_data_at_z(). "
              "This means the time-sampling grid was not initialized for this output configuration "
              "(likely transfer-only output).",
              ppt->error_message);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_schi",ppt->has_source_delta_chi);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      /* spectator field chi velocity transfer (theta_chi) */
      class_store_columntitle(titles,"t_schi",ppt->has_source_theta_chi);
...
int perturbations_indices(
  ...
  /* If the user requested the alpha-alpha spectrum, make sure the alpha source is enabled */
  if (ppt->has_cl_aa == _TRUE_) {
    ppt->has_source_alpha = _TRUE_;
    /* Treat alpha like a CMB-type line-of-sight source so that source sampling starts early enough */
    ppt->has_cmb = _TRUE_;
  }
  ...
  if ((ppt->has_cl_cmb_lensing_potential == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
    ppt->has_lss = _TRUE_;
    ppt->has_source_phi_plus_psi = _TRUE_;
  }

  if ((ppt->has_pk_matter == _TRUE_) || (ppt->has_nl_corrections_based_on_delta_m == _TRUE_)) {
    ppt->has_lss = _TRUE_;
  ...
  if (ppt->has_density_transfers == _TRUE_) {
    ...
    if (pba->has_cdm == _TRUE_) ppt->has_source_delta_cdm = _TRUE_;
    if (pba->has_idm == _TRUE_) ppt->has_source_delta_idm = _TRUE_;
    if (pba->has_dcdm == _TRUE_) ppt->has_source_delta_dcdm = _TRUE_;
    if (pba->has_fld == _TRUE_) ppt->has_source_delta_fld = _TRUE_;
    if (pba->has_scf == _TRUE_) ppt->has_source_delta_scf = _TRUE_;
    if (pba->has_chi == _TRUE_) ppt->has_source_delta_chi = _TRUE_;
    if (pba->has_ur == _TRUE_) ppt->has_source_delta_ur = _TRUE_;
    if (pba->has_idr == _TRUE_) ppt->has_source_delta_idr = _TRUE_;
    if (pba->has_dr == _TRUE_) ppt->has_source_delta_dr = _TRUE_;
    if (pba->has_ncdm == _TRUE_) ppt->has_source_delta_ncdm = _TRUE_;
  ...
  if (ppt->has_velocity_transfers == _TRUE_) {
    ...
    if ((pba->has_cdm == _TRUE_) && (ppt->gauge != synchronous)) ppt->has_source_theta_cdm = _TRUE_;
    if (pba->has_idm == _TRUE_) ppt->has_source_theta_idm = _TRUE_;
    if (pba->has_dcdm == _TRUE_) ppt->has_source_theta_dcdm = _TRUE_;
    if (pba->has_fld == _TRUE_) ppt->has_source_theta_fld = _TRUE_;
    if (pba->has_scf == _TRUE_) ppt->has_source_theta_scf = _TRUE_;
    if (pba->has_chi == _TRUE_) ppt->has_source_theta_chi = _TRUE_;
    if (pba->has_ur == _TRUE_) ppt->has_source_theta_ur = _TRUE_;
    if (pba->has_idr == _TRUE_) ppt->has_source_theta_idr = _TRUE_;
    if (pba->has_dr == _TRUE_) ppt->has_source_theta_dr = _TRUE_;
    if (pba->has_ncdm == _TRUE_) ppt->has_source_theta_ncdm = _TRUE_;
  }
  ...
  class_define_index(ppt->index_tp_alpha_reco,    ppt->has_source_alpha,          index_type,1);
  class_define_index(ppt->index_tp_alpha_reio,    ppt->has_source_alpha,          index_type,1);
...
int perturbations_timesampling_for_sources(
  ...
  */
  ...
  class_alloc(ppt->ln_tau,
              ppt->ln_tau_size * sizeof(double),
              ppt->error_message);

  /* tau at z=0 is today (conformal_age). This makes the range checks in
     perturbations_output_data_at_z() work: asking for z>0 triggers the
     intended error instead of a segfault. */
  ppt->ln_tau[0] = log(pba->conformal_age);
}
...
int perturbations_get_k_list(
  ...
  if (ppt->has_scalars == _TRUE_) {
    ...
    /* --- User-side hard cap for transfer/P(k) runs (no Cl's) ---
       If the run does not request any Cl's, but does request matter/density/velocity transfers
       (or P(k)), then the only physically relevant cap is ppt->k_max_for_pk, which is set from
       P_k_max_1/Mpc. In that case, enforce it strictly so CLASS never extends the k-list beyond
       what the user asked for.

       Note: If you *do* request Cl's (ppt->has_cls==_TRUE_), do NOT cap here because the Cl
       accuracy requirements can legitimately demand a larger k_max.
    */
    if (ppt->has_cls == _FALSE_) {
      if ((ppt->has_pk_matter == _TRUE_) ||
          (ppt->has_density_transfers == _TRUE_) ||
          (ppt->has_velocity_transfers == _TRUE_)) {
        if (ppt->k_max_for_pk > 0.) {
          k_max = MIN(k_max, ppt->k_max_for_pk);
          k_max_cmb[ppt->index_md_scalars] = MIN(k_max_cmb[ppt->index_md_scalars], ppt->k_max_for_pk);
          k_max_cl[ppt->index_md_scalars]  = MIN(k_max_cl[ppt->index_md_scalars],  ppt->k_max_for_pk);
        }
      }
    }
  ...
  */
  ...
  /* Optional diagnostic: print final k-range once per run (AFTER it is computed) */
  if (ppt->perturbations_verbose > 0) {
    fprintf(stdout,
            "[perturbations_get_k_list DONE] has_cls=%d has_pk=%d has_dTk=%d has_vTk=%d k_max_for_pk=%e k_min=%e k_max=%e\n",
            (int)ppt->has_cls,
            (int)ppt->has_pk_matter,
            (int)ppt->has_density_transfers,
            (int)ppt->has_velocity_transfers,
            ppt->k_max_for_pk,
            ppt->k_min,
            ppt->k_max);
    fflush(stdout);
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta_chi,  pba->has_chi, index_pt, 1); /* chi density perturbation */
    class_define_index(ppv->index_pt_deltap_chi, pba->has_chi, index_pt, 1); /* chi velocity-like perturbation */
    ...
    */
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      /* spectator chi: Newtonian gauge transformation */
      if (pba->has_chi == _TRUE_) {

        /* delta_chi -> delta_chi + chi0' * alpha */
        ppw->pv->y[ppw->pv->index_pt_delta_chi] += alpha * ppw->pvecback[pba->index_bg_chi0_prime];

        /* delta_chi' -> delta_chi' + chi0'' * alpha + chi0' * alpha'
          with chi0'' = -2(a'/a)chi0' - a^2 dV_chi(chi0)   (background KG)
        */
        ppw->pv->y[ppw->pv->index_pt_deltap_chi] +=
          (-2.*a_prime_over_a*alpha*ppw->pvecback[pba->index_bg_chi0_prime]
          -a*a*dV_chi(pba, ppw->pvecback[pba->index_bg_chi0])*alpha
          +ppw->pvecback[pba->index_bg_chi0_prime]*alpha_prime);
      }
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_p == _TRUE_) {
      ...
      double g_reco_p = 0.0;
      double g_reio_p = 0.0;
      double g_peak_reco_p = -1.0;
      double g_peak_reio_p = -1.0;
      double z_peak_reco_p = -1.0;
      double z_peak_reio_p = -1.0;
      double g_valley_p;
      double z_split_p;
      int index_peak_reco_p = -1;
      int index_peak_reio_p = -1;
      int index_start_p;
      int index_end_p;
      int index_valley_p;

      const double z_p = 1.0 / pvecback[pba->index_bg_a] - 1.0;

      for (index_th = 0; index_th < pth->tt_size; index_th++) {
        const double g_tab =
          pth->thermodynamics_table[index_th*pth->th_size + pth->index_th_g];

        if (g_tab > g_peak_reco_p) {
          g_peak_reco_p = g_tab;
          z_peak_reco_p = pth->z_table[index_th];
          index_peak_reco_p = index_th;
        }
      }
      ...
      for (index_th = 1; index_th < pth->tt_size - 1; index_th++) {
        ...
        if ((z_tab < z_peak_reco_p) && (g_tab >= g_prev) && (g_tab > g_next)) {
          if (g_tab > g_peak_reio_p) {
            g_peak_reio_p = g_tab;
            z_peak_reio_p = z_tab;
            index_peak_reio_p = index_th;
          }
        }
      }

      if (index_peak_reio_p < 0) {
        for (index_th = 0; index_th < pth->tt_size; index_th++) {
          const double z_tab = pth->z_table[index_th];
          const double g_tab =
            pth->thermodynamics_table[index_th*pth->th_size + pth->index_th_g];

          if ((z_tab < z_peak_reco_p) && (g_tab > g_peak_reio_p)) {
            g_peak_reio_p = g_tab;
            z_peak_reio_p = z_tab;
            index_peak_reio_p = index_th;
          }
      ...
      class_test((index_peak_reco_p < 0) || (index_peak_reio_p < 0),
                 ppt->error_message,
                 "Could not identify recombination and reionization peaks in the visibility function for polarization splitting.");

      index_start_p = MIN(index_peak_reco_p,index_peak_reio_p);
      index_end_p   = MAX(index_peak_reco_p,index_peak_reio_p);
      index_valley_p = index_start_p;
      g_valley_p = pth->thermodynamics_table[index_start_p*pth->th_size + pth->index_th_g];

      for (index_th = index_start_p; index_th <= index_end_p; index_th++) {
        const double g_tab =
          pth->thermodynamics_table[index_th*pth->th_size + pth->index_th_g];

        if (g_tab < g_valley_p) {
          g_valley_p = g_tab;
          index_valley_p = index_th;
        }
      }

      z_split_p = pth->z_table[index_valley_p];
      ...
      _set_source_(ppt->index_tp_p_reco) = sqrt(6.) * g_reco_p * P;
      _set_source_(ppt->index_tp_p_reio) = sqrt(6.) * g_reio_p * P;
    ...
    /* delta_chi (spectator scalar field chi) */
  ...
  if (ppt->has_source_alpha == _TRUE_) {
      ...
      /*
      * Build the full birefringence source alpha and split it here into
      * recombination and reionization contributions using the actual
      * visibility profile g(z), not fixed redshift windows.
      *
      ...
      */
      ...
      {
        /* Current redshift of the source evaluation point */
        const double z_alpha = 1.0 / pvecback[pba->index_bg_a] - 1.0;

        /* Visibility-peak information */
        double z_peak_reco = -1.0;
        double z_peak_reio = -1.0;
        ...
        /* Indices of the two visibility peaks and of the valley between them */
        int index_peak_reco = -1;
        int index_peak_reio = -1;
        int index_start;
        int index_end;
        int index_valley;
        ...
        /*
        * Step 1:
        * Find the global maximum of g(z). This is the recombination peak.
        */
        for (index_th = 0; index_th < pth->tt_size; index_th++) {
          const double g_tab =
            pth->thermodynamics_table[index_th*pth->th_size + pth->index_th_g];

          if (g_tab > g_peak_reco) {
            g_peak_reco = g_tab;
            z_peak_reco = pth->z_table[index_th];
        ...
        /*
        * Step 2:
        * Find the strongest lower-z local maximum of g(z).
        * This is the reionization peak.
        */
        for (index_th = 1; index_th < pth->tt_size - 1; index_th++) {
          const double z_tab = pth->z_table[index_th];
          const double g_prev =
          ...
          if ((z_tab < z_peak_reco) && (g_tab >= g_prev) && (g_tab > g_next)) {
            if (g_tab > g_peak_reio) {
              g_peak_reio = g_tab;
              z_peak_reio = z_tab;
        ...
        /*
        * Fallback in case the reionization bump is broad enough that the simple
        * local-maximum test misses it.
        */
        ...
        */
        ...
        _set_source_(ppt->index_tp_alpha_reco) =
          (-0.5 * ppr->lambda_over_f) * g_reco * delta_chi_for_alpha;

        _set_source_(ppt->index_tp_alpha_reio) =
    ...
    /* theta_chi (spectator scalar field chi) */
    if (ppt->has_source_theta_chi == _TRUE_) {
      _set_source_(ppt->index_tp_theta_chi) = y[ppw->pv->index_pt_deltap_chi]
        + theta_shift; /* N-body gauge correction, consistent with other theta_i */
    }
